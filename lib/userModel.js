...
async function createUser({ correoUniversitario, contrasena, nombre, rol = 'estudiante', telefono = null, carrera = '', intereses = [] }) {
  await ensureConnection();
  if (useFallback) return fallback.createUser({ correoUniversitario, contrasena, nombre, rol, telefono, carrera, intereses });
  const hash = await bcrypt.hash(contrasena, 10);
  const u = new Usuario({ correoUniversitario, contrasena: hash, nombre, rol, telefono, carrera, intereses });
  await u.save();
  return u.toObject();
}
...
async function updateProfile(userId, profileData = {}) {
  ...
  const allowedFields = ['nombre', 'telefono', 'carrera', 'intereses'];
  const payload = {};
...
module.exports = { findByCorreo, findById, createUser, createUserFromHash, comparePassword, updatePassword, updateProfile };
