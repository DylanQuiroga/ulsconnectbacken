...
router.post('/create', ensureAuth, ensureRole(['admin', 'staff']), async (req, res) => {
  try {
    const actividadId = req.body && (req.body.actividadId || req.body.actividad || req.body.activityId);
...
router.post('/take', ensureAuth, ensureRole(['admin', 'staff']), async (req, res) => {
  try {
    const attendanceId = req.body && (req.body.attendanceId || req.body.id || req.body.attendance);
    if (!attendanceId) return res.status(400).json({ success: false, message: 'attendanceId requerido' });

    const payload = {
      presentes: Array.isArray(req.body.presentes) ? req.body.presentes : (Array.isArray(req.body.presentes) ? req.body.presentes : (req.body.presentes || req.body.presentes === 0 ? req.body.presentes : [])),
      ausentes: Array.isArray(req.body.ausentes) ? req.body.ausentes : [],
      justificadas: Array.isArray(req.body.justificadas) ? req.body.justificadas : []
    };
