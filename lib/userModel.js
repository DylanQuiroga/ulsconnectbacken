// Operaciones de usuario con MongoDB y fallback a archivo
...
async function adjustScore(userId, delta, options = {}) {
  if (!userId) throw new Error('User ID is required');
  const change = Number(delta);
  if (!Number.isFinite(change) || change === 0) throw new Error('delta debe ser un numero distinto de cero');

  const {
    motivo = '',
    actividadId = null,
    registradoPor = null,
    dedupeByActivity = false
  } = options;

  await ensureConnection();

  if (useFallback && typeof fallback.adjustScore === 'function') {
    return fallback.adjustScore(userId, change, { motivo, actividadId, registradoPor, dedupeByActivity });
  }
  ...
  if (dedupeByActivity && actividadId) {
    query['historialPuntos.actividad'] = { $ne: actividadId };
  }
  ...
  if (!updated) {
    const existing = await Usuario.findById(userId).select('-contrasena').lean();
    return { applied: false, user: existing };
  }
...
async function getScore(userId, limit = 20) {
  if (!userId) throw new Error('User ID is required');
  const capped = Math.max(0, Math.min(parseInt(limit, 10) || 20, 100));
  await ensureConnection();

  if (useFallback && typeof fallback.getScore === 'function') {
    return fallback.getScore(userId, capped);
  }
  ...
  const user = await Usuario.findById(
    ...
    {
      puntos: 1,
      historialPuntos: { $slice: capped },
      nombre: 1,
      correoUniversitario: 1,
      rol: 1
    }
  ).lean();
  ...
  return {
    ...
    historial: Array.isArray(user.historialPuntos) ? user.historialPuntos : [],
    usuario: {
      _id: user._id,
      nombre: user.nombre,
...
async function getLeaderboard(limit = 10) {
  const capped = Math.max(1, Math.min(parseInt(limit, 10) || 10, 100));
  await ensureConnection();

  if (useFallback && typeof fallback.getLeaderboard === 'function') {
    return fallback.getLeaderboard(capped);
  }

  return Usuario.find({ rol: 'estudiante' })
    .sort({ puntos: -1, creadoEn: 1 })
    .limit(capped)
    .select('-contrasena')
    .lean();
}
