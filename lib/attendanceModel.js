...
class AttendanceModel {
  ...
  static async createAttendanceList(actividadId, sessionUserId) {
    try {
      ...
      // Ensure there's not already an attendance list for this activity
      const existing = await Attendance.findOne({ actividad: actividadId });
      if (existing) { return await Attendance.findById(existing._id).populate('actividad').populate('inscripciones.usuario').populate('registradoPor'); }
